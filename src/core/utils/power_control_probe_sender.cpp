...
void PowerControlProbeSender::Start(void)
{
    ...
    otLogDebgUtil("TPC: probe sender start with interval: %d", TPC_LINKMETRIC_TIMER_CHECK_INTERVAL);
#endif
...
#if OPENTHREAD_CONFIG_POWER_CONTROL_DEBUG_ENABLE
    otLogInfoUtil("TPC: probe sender set interval to: %d seconds", aSeconds);
#endif
...
void PowerControlProbeSender::HandleTimer(void)
{
    ...
    if (Get<Mle::Mle>().IsAttached())
    {
        while (otThreadGetNextNeighborInfo(&GetInstance(), &iterator, &neighborInfo) == kErrorNone)
        {
            ...
            if (neighbor != nullptr)
            {
                ...
                {
#if OPENTHREAD_CONFIG_POWER_CONTROL_DEBUG_ENABLE
                    otLogDebgUtil("TPC: probe sender sending query to: %02x ", neighborInfo.mRloc16);
#endif
                    ot::Mac::ExtAddress mac;
    ...
    #if OPENTHREAD_CONFIG_POWER_CONTROL_DEBUG_ENABLE
         otLogDebgUtil("TPC: probe sender sending query to the parent");
    #endif
    ot::Mac::ExtAddress mac;
